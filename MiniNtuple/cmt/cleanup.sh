...
${CMTROOT}/${CMTBIN}/cmt.exe cleanup -sh -pack=MiniNtuple -version=MiniNtuple-HEAD -path=/afs/cern.ch/user/o/othrif/testarea/20.7.3  $* >${cmtMiniNtupletempfile}
if test $? != 0 ; then
  echo >&2 "${CMTROOT}/${CMTBIN}/cmt.exe cleanup -sh -pack=MiniNtuple -version=MiniNtuple-HEAD -path=/afs/cern.ch/user/o/othrif/testarea/20.7.3  $* >${cmtMiniNtupletempfile}"
  cmtcleanupstatus=2
