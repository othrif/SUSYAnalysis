...
void getData_Signature(distrSignature& set, string tag, const char* signal_name, Int_t ind, const char* syst_tag) {
  ...
  //  getData_Region(set.SR1, "SR1b_DD_lowmMeff_" + tag, "", -1, syst_tag);
  getData_Region(set.SR1, region +"_"+ tag, "", -1, syst_tag);
...
void getData_SigSignature(distrSigSignature& set, string tag, const char* signal_name, Int_t ind, const char* syst_tag) {
  ...
  getData_SigRegion(set.SR1, "SR1b_DD_lowmMeff_" + tag, "", -1, syst_tag);
...
void getData_SigSet(distrSigSet& set, string tag, const char* signal_name, Int_t ind, const char* syst_tag){
  ...
  const string files[N_SIG] = {"4t"};//{"ttn1_1200_5000_1", "ttn1_1200_5000_600", "ttn1_1200_5000_855"}; 
...
void setFakeRates_Set(distrSet& set, Double_t cf, Double_t sf1, Double_t sf2, Double_t sf3, Double_t sf4) {
  ...
  set.charge_flip = cf*fug;
  set.FR_hf_e = sf1*fug;
  set.FR_lf_e = sf2*fug;
  set.FR_hf_mu = sf3*fug;
  set.FR_lf_mu = sf4*fug;
...
void drawAVariable_test(THStack* &st, TH1D* hist[N_FAKE], TH1D* sig[N_SIG], TObjArray *paired, TObjArray *unpaired, const char* fit){
  ...
  //  sprintf(my_extension, "/afs/cern.ch/work/o/othrif/workarea/results/v52/Plots/%s.pdf", hist[0]->GetName());
  sprintf(my_extension, "/afs/cern.ch/work/o/othrif/workarea/results/v52/Plots/result.pdf");
...
legendHeader += ", VRWZ4j, ";
	...
	char fpath[512];
	char my_extension[256] = "";
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v52/Plots/%s", folder);
	int res = mkdir(fpath, 0700);
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v52/Plots/%s/%s-fit", folder, fit_type.c_str());
	res = mkdir(fpath, 0700);
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v52/Plots/%s/%s-fit/normalized", folder, fit_type.c_str());
	res = mkdir(fpath, 0700);
	if(ilog){
...
void drawAVariable_all(THStack* &st, TH1D* hist[N_FAKE], TH1D* sig[N_SIG], TObjArray *paired, TObjArray *unpaired, const char* fit){  
  ...
  if(blind)
	hist[0]->SetBinContent(hist[0]->GetNbinsX(),0);
  
  /*  for (int k = 1; k <= hist[0]->GetNbinsX(); k++){
	cout << "bin " << k << " " << hist[0]->GetBinContent(k) << std::endl;
	}*/
  
  // oth
  hist[2+1]->SetBinContent(hist[0]->GetNbinsX(),3.11);
  hist[2+2]->SetBinContent(hist[0]->GetNbinsX(),2.11);
  hist[2+3]->SetBinContent(hist[0]->GetNbinsX(),2.);
  ...
  for (int ilog=0; ilog<1; ilog++){
	...
	// oth
	err_brs->SetBinContent(err_brs->GetNbinsX(),20);
	err_brs->SetBinError(err_brs->GetNbinsX(),5);
		/*	err_brs->SetBinContent(err_brs->GetNbinsX(),2 );*/
	//	err_brs->SetBinContent(err_brs->GetNbinsX()-1,5 );
	//	err_brs->SetBinError(err_brs->GetNbinsX()-1,0.75 );
	...
	if(draw_ratio){
	  ...
	  */
      ...
      rat->GetYaxis()->SetLabelSize(0.09);

      rat->GetXaxis()->SetTitleSize(0.09);
      rat->GetXaxis()->SetTitleOffset(1.);
      rat->GetYaxis()->SetTitleSize(0.09);
	  ...
	  if(strstr(hist[0]->GetName(), "Meff"))
		rat->GetXaxis()->SetNdivisions(508);
	...
	if( true /*strstr(hist[0]->GetName(), "SR") == NULL */){
	  ...
	  char fpath[512];
	  char my_extension[256] = "";
	  char my_extension_result[256] = "";
	  sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v52/Plots/%s", folder);
	  int res = mkdir(fpath, 0700);
	  sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v52/Plots/%s/%s-fit", folder, fit_type.c_str());
	  res = mkdir(fpath, 0700);
	  sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v52/Plots/%s/%s-fit/all", folder, fit_type.c_str());
	  res = mkdir(fpath, 0700);
	  if(ilog){
		sprintf(fpath, "%s/log", fpath);
		res = mkdir(fpath, 0700);
		...
		res = mkdir(fpath, 0700);
		//		sprintf(my_extension, "%s/%s_log.pdf", fpath, hist[0]->GetName());
		sprintf(my_extension, "%s/result_log.pdf", fpath);
	  }
	  else{
		sprintf(fpath, "%s/linear", fpath);
		res = mkdir(fpath, 0700);
		...
		res = mkdir(fpath, 0700);
		//		sprintf(my_extension, "%s/%s.pdf", fpath, hist[0]->GetName());
		sprintf(my_extension, "/afs/cern.ch/user/o/othrif/Desktop/nm1/%s.pdf", hist[0]->GetName());
		sprintf(my_extension_result, "/afs/cern.ch/user/o/othrif/Desktop/result.pdf", fpath);
	  }
	  myPlots->Print(my_extension);
	  myPlots->Print(my_extension_result);
	}
	delete myPlots;
  }
...
void drawAVariable_merged(THStack* &st, TH1D* hist[N_FAKE], TH1D* sig[N_SIG], TObjArray *paired, TObjArray *unpaired, const char* fit){
  ...
  for (int ilog=0; ilog<2; ilog++){
	...
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v52/Plots/%s/%s-fit", folder, fit_type.c_str());
	res = mkdir(fpath, 0700);
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v52/Plots/%s/%s-fit/merged", folder, fit_type.c_str());
	res = mkdir(fpath, 0700);
	if(ilog){
	  sprintf(fpath, "%s/log", fpath);
	  res = mkdir(fpath, 0700);
...
void CalcBackgrounds( TH1D* hist[N_FAKE], TObjArray *paired, TObjArray *unpaired, distrSet &my_set) {
  ...
  my_set.fakes_systup = sqrt(pow(fakes_systup,2)+pow(fakes_mean*.75,2));
  my_set.fakes_systdn = sqrt(pow(fakes_systdn,2)+pow(fakes_mean*.75,2));
...
void printBackgrounds(distrAll *my_set){
  ...
  char logfile[512];
  sprintf(logfile,"/afs/cern.ch/work/o/othrif/workarea/results/v52/Plots/%s/yields_%s.txt", folder, fit_type.c_str());
  FILE *fp;
  fp=fopen(logfile, "w+");
  ...
  //  fprintf(fp,"\n &   SR1  \\\\");
  printf("\nFakes & $%.2f \\pm %.2f \\pm %.2f$  combined error $ %.2f $\\\\",  
		  my_set->SS3L.SR1.fs_comb.fakes_mean, my_set->SS3L.SR1.fs_comb.fakes_stat, std::max(my_set->SS3L.SR1.fs_comb.fakes_systup, my_set->SS3L.SR1.fs_comb.fakes_systdn), sqrt( pow(my_set->SS3L.SR1.fs_comb.fakes_stat,2)+ pow(std::max(my_set->SS3L.SR1.fs_comb.fakes_systup, my_set->SS3L.SR1.fs_comb.fakes_systdn),2))
		  );
  printf("\nCharge flip & $%.2f \\pm %.2f \\pm %.2f$  combined error $ %.2f $\\\\",  
		  my_set->SS3L.SR1.fs_comb.backgr_mean[charge_flip_ind_fit],my_set->SS3L.SR1.fs_comb.backgr_stat[charge_flip_ind_fit], std::max(my_set->SS3L.SR1.fs_comb.backgr_systup[charge_flip_ind_fit], my_set->SS3L.SR1.fs_comb.backgr_systdn[charge_flip_ind_fit]), sqrt(pow( my_set->SS3L.SR1.fs_comb.backgr_stat[charge_flip_ind_fit],2) + pow( std::max(my_set->SS3L.SR1.fs_comb.backgr_systup[charge_flip_ind_fit], my_set->SS3L.SR1.fs_comb.backgr_systdn[charge_flip_ind_fit]),2))
		  );

  printf("\nTotal & $%.2f \\pm %.2f \\pm %.2f$  combined error $ %.2f $\\\\",  
		  my_set->SS3L.SR1.fs_comb.backgr_mean[N_DataMC],my_set->SS3L.SR1.fs_comb.backgr_stat[N_DataMC],std::max(my_set->SS3L.SR1.fs_comb.backgr_systup[N_DataMC], my_set->SS3L.SR1.fs_comb.backgr_systdn[N_DataMC]),sqrt(pow( my_set->SS3L.SR1.fs_comb.backgr_stat[N_DataMC],2) + pow( std::max(my_set->SS3L.SR1.fs_comb.backgr_systup[N_DataMC], my_set->SS3L.SR1.fs_comb.backgr_systdn[N_DataMC]),2))
		  );
...
int main(int argc, char **argv ) {
  ...
  for(int itr=1; itr<2; itr++){ 
	...
	*/
	...
	//luminosity
	Float_t c_lumi_up = 1. + 0.50;//0.045;
	Float_t c_lumi_dn = 1. - 0.50;//0.045;
	if(!test_me){
