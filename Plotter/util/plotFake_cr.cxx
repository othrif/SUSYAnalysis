...
void getData_Region(distrRegion& set, string tag, const char* signal_name, Int_t ind, const char* syst_tag) {
  ...
  getData_Set(set.fs_elel, "comb_" + tag, "", -1, syst_tag);
...
void getData_SigRegion(distrSigRegion& set, string tag, const char* signal_name, Int_t ind, const char* syst_tag) {
  ...
  getData_SigSet(set.fs_elel, "comb_" + tag, "", -1, syst_tag);
...
void draw_Signature(distrSignature &set, distrSigSignature &sig, distrSignature* paired[N_unc_paired], distrSignature* unpaired[N_unc_unpaired], const char* fit){
  ...
  distrRegion* paired_VRWZ[N_unc_paired];
  distrRegion* unpaired_VRWZ[N_unc_unpaired];
...
void drawAVariable_normalized(THStack* &st, TH1D* hist[N_FAKE], TH1D* sig[N_SIG], TObjArray *paired, TObjArray *unpaired, const char* fit){
  ...
  for (int ilog=0; ilog<2; ilog++){
	...
	if (strstr(hist[0]->GetName(), "_VRWW_") != NULL)
	  legendHeader += ", VRWW, ";
	if (strstr(hist[0]->GetName(), "_VRWZ_") != NULL)
	  legendHeader += ", VRWZ, ";
...
void drawAVariable_all(THStack* &st, TH1D* hist[N_FAKE], TH1D* sig[N_SIG], TObjArray *paired, TObjArray *unpaired, const char* fit){  
  ...
  for (int ilog=0; ilog<1; ilog++){
	...
	if(draw_ratio){
	  ...
	  TH1DaxisRatio->SetMinimum(0.4);
	  TH1DaxisRatio->SetMaximum(1.6);
	  ...
	  line->SetLineWidth(2);
	  line->Draw();
	  line = new TLine(myPad1->GetUxmin(),1.2,myPad1->GetUxmax(),1.2);
	  line->SetLineStyle(2);
	  line->SetLineWidth(2);
	  line->Draw();
	  line = new TLine(myPad1->GetUxmin(),1.4,myPad1->GetUxmax(),1.4);
	  line->SetLineStyle(2);
	  line->SetLineWidth(2);
	  line->Draw();
	  line = new TLine(myPad1->GetUxmin(),0.8,myPad1->GetUxmax(),0.8);
	  line->SetLineStyle(2);
	  line->SetLineWidth(2);
	  line->Draw();
	  line = new TLine(myPad1->GetUxmin(),0.6,myPad1->GetUxmax(),0.6);
	  line->SetLineStyle(2);
	  line->SetLineWidth(2);
	  line->Draw();
	...
	if( strstr(hist[0]->GetName(), "SR") == NULL ){
	  ...
	  if(ilog){
		...
		res = mkdir(fpath, 0700);
		sprintf(my_extension, "%s/%s.pdf", fpath, hist[0]->GetName());
		//		sprintf(my_extension, "%s/%s.C", fpath, hist[0]->GetName());
	  }
	  myPlots->Print(my_extension);
	}
	delete myPlots;
...
TH1D* errorBars(THStack* central, TObjArray *paired, TObjArray *unpaired) {
  ...
  for (int i = 1; i <= N_bins; i++) {
	...
	pos_err = pow(stat_error, 2) + pow(0.1*mean_value, 2);
	neg_err = pow(stat_error, 2)+ pow(0.1*mean_value, 2);
...
void printBackgrounds(distrAll *my_set){
  ...
  fprintf(fp,"\n\n$ee + e\\mu + \\mu\\mu$ channel\\\\");
  fprintf(fp,"\n \\begin{tabular}{ l l l } ");
  fprintf(fp,"\n & VRWW & VRWZ & VRttW & VRttZ \\\\");
  fprintf(fp,"\nFakes & %.2f$\\pm$%.2f$^{+%.2f}_{-%.2f}$ & %.2f$\\pm$%.2f$^{+%.2f}_{-%.2f}$ & %.2f$\\pm$%.2f$^{+%.2f}_{-%.2f}$ & %.2f$\\pm$%.2f$^{+%.2f}_{-%.2f}$  \\\\",  
  ...
  my_set->SS3L.VRWZ.fs_comb.fakes_mean, my_set->SS3L.VRWZ.fs_comb.fakes_stat, my_set->SS3L.VRWZ.fs_comb.fakes_systup, my_set->SS3L.VRWZ.fs_comb.fakes_systdn,
  ...
  fprintf(fp,"\nCharge flip & %.2f$\\pm$%.2f$^{+%.2f}_{-%.2f}$ & %.2f$\\pm$%.2f$^{+%.2f}_{-%.2f}$ & %.2f$\\pm$%.2f$^{+%.2f}_{-%.2f}$ & %.2f$\\pm$%.2f$^{+%.2f}_{-%.2f}$\\\\",
  ...
  my_set->SS3L.VRWZ.fs_comb.backgr_mean[charge_flip_ind_fit], my_set->SS3L.VRWZ.fs_comb.backgr_stat[charge_flip_ind_fit], my_set->SS3L.VRWZ.fs_comb.backgr_systup[charge_flip_ind_fit], my_set->SS3L.VRWZ.fs_comb.backgr_systdn[charge_flip_ind_fit],
  ...
  my_set->SS3L.VRWZ.fs_comb.backgr_mean[1], my_set->SS3L.VRWZ.fs_comb.backgr_stat[1], my_set->SS3L.VRWZ.fs_comb.backgr_systup[1], my_set->SS3L.VRWZ.fs_comb.backgr_systdn[1],
  ...
  my_set->SS3L.VRWZ.fs_comb.backgr_mean[N_DataMC], my_set->SS3L.VRWZ.fs_comb.backgr_stat[N_DataMC], my_set->SS3L.VRWZ.fs_comb.backgr_systup[N_DataMC], my_set->SS3L.VRWZ.fs_comb.backgr_systdn[N_DataMC],
  ...
  //		  my_set->SS3L.VRWZ.fs_comb.NJETS[0]->Integral(0, n_bins+1), 
  //		  my_set->SS3L.SRttW.fs_comb.NJETS[0]->Integral(0, n_bins+1), 
...
int main(int argc, char **argv ) {
  ...
  for(int itr=0; itr<2; itr++){ 
	...
	*/
	...
	//luminosity
	Float_t c_lumi_up = 1. + 0.145;//0.045;
	Float_t c_lumi_dn = 1. - 0.145;//0.045;
