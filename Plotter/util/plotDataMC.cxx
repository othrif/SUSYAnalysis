...
void getData_Signature(distrSignature& set, string tag, const char* signal_name, Int_t ind, const char* syst_tag) {
  ...
  if(!test_me){  getData_Region(set.PRE, "PRE_" + tag, "", -1, syst_tag);
	getData_Region(set.CR0b, "CR0b_" + tag, "", -1, syst_tag);
	getData_Region(set.CR1b, "CR1b_" + tag, "", -1, syst_tag);
	getData_Region(set.CR2b, "VRttW_" + tag, "", -1, syst_tag);
...
void getData_Region(distrRegion& set, string tag, const char* signal_name, Int_t ind, const char* syst_tag) {
  ...
  if(!test_me){  getData_Set(set.fs_elel, "ee_" + tag, "", -1, syst_tag);
	getData_Set(set.fs_elmu, "em_" + tag, "", -1, syst_tag);
	getData_Set(set.fs_mumu, "mm_" + tag, "", -1, syst_tag);}
  getData_Set(set.fs_comb, "em_" + tag, "", -1, syst_tag);
...
void getData_Set(distrSet& set, string tag, const char* signal_name, Int_t ind, const char* syst_tag){
  ...
  // powheg
  // pythia8
  string temp_mc, ttbar_mc, zjets_mc, wjets_mc, vv_mc="";

  if (strstr(temp_file, "sherpa") != NULL)
	temp_mc = "sherpa"; 
  else if (strstr(temp_file, "powheg") != NULL)
	{
	  temp_mc = "powhegpythia8"; 
	}

  ttbar_mc = 	temp_mc+"TTBar_mc";
  zjets_mc = 	temp_mc+"Zjets_mc";
  wjets_mc = 	temp_mc+"Wjets_mc";

  if (strstr(temp_file, "powheg") != NULL)
	ttbar_mc = "powhegpythia6TTBar_mc";

  if (strstr(temp_file, "pythia8") != NULL)
	ttbar_mc = "mgTTBar_mc";
  
  if (strstr(temp_file, "ximo") != NULL){
	ttbar_mc = 	"powhegpythia6TTBar_mc";
	zjets_mc = 	"sherpaZjets_mc";
	//	wjets_mc = 	"sherpaWjets_mc";
	wjets_mc = 	"powhegpythia8Wjets_mc";
  }
	
  if (strstr(temp_file, "old") != NULL)
	vv_mc = "old"; 


  // Input Files 
  const string processes[N_DataMC] = {"data", "ttW_mc", "ttZ_mc", "WW_mc", 
									  "WZ"+vv_mc+"_mc", "ZZ"+vv_mc+"_mc", 
									  "tth_mc", "Vh_mc", 
									  ...
									  //									temp_mc+"Zjets_mc",
									  //									temp_mc+"Wjets_mc"
									  /*"powhegpythia6TTBar_mc",   
										"sherpaZjets_mc",     
										"powhegpythia8Wjets_mc"  */
									  /*"sherpaTTBar_mc",
										"sherpaZjets_mc",
										"sherpaWjets_mc"
										"powhegpythia6TTBar_mc", 
										"powhegpythia8Zjets_mc",
										"powhegpythia8Wjets_mc"
										"mgTTBar_mc", 
										"mgZjets_mc",
										"mgWjets_mc"*/
  ...
  for (int i = 0; i < N_DataMC; i++) {
	// NJETS
	std::sprintf(temp_tag, "Meff_%s_%s", processes[i].c_str(),tag.data());
	//	std::sprintf(temp_tag, "el2_pt_%s_%s", processes[i].c_str(),tag.data());
	//	std::sprintf(temp_tag, "Mll_%s_%s", processes[i].c_str(),tag.data());
	...
	//std::cout << temp_tag << std::endl;
	//	std::cout << temp_tag << " " << set.NJETS[i]->Integral() << std::endl;
	...
	if(!test_me){
	  // pt_jet1
	  std::sprintf(temp_tag, "pt_jet1_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // pt_jet2
	  std::sprintf(temp_tag, "pt_jet2_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // pt_jet3
	  std::sprintf(temp_tag, "pt_jet3_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  if(reduced){
		// pt_jet4
		std::sprintf(temp_tag, "pt_jet4_%s_%s", processes[i].c_str(),tag.data());
		...
		// pt_jet5
		std::sprintf(temp_tag, "pt_jet5_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // NBJETS
	  std::sprintf(temp_tag, "NBJETS_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // pt_bjet1
	  std::sprintf(temp_tag, "pt_bjet1_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // pt_bjet2
	  std::sprintf(temp_tag, "pt_bjet2_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // pt_bjet3
	  std::sprintf(temp_tag, "pt_bjet3_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  if(reduced){
		// pt_bjet4
		std::sprintf(temp_tag, "pt_bjet4_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // mu1_pt
	  std::sprintf(temp_tag, "mu1_pt_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // mu2_pt
	  std::sprintf(temp_tag, "mu2_pt_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // mu3_pt
	  std::sprintf(temp_tag, "mu3_pt_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // mu_eta
	  std::sprintf(temp_tag, "mu_eta_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // el1_pt
	  std::sprintf(temp_tag, "el1_pt_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // el3_pt
	  std::sprintf(temp_tag, "el3_pt_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // el_eta
	  std::sprintf(temp_tag, "el_eta_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // MET
	  std::sprintf(temp_tag, "MET_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // HT
	  std::sprintf(temp_tag, "HT_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // mT
	  std::sprintf(temp_tag, "mT_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // mT_min
	  std::sprintf(temp_tag, "mT_min_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  if(reduced){
		// MZ
		std::sprintf(temp_tag, "MZ_%s_%s", processes[i].c_str(),tag.data());
		...
		// Mlll
		std::sprintf(temp_tag, "Mlll_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // Mll
	  std::sprintf(temp_tag, "Mll_%s_%s", processes[i].c_str(),tag.data());
	  set.Mll[i] =(TH1D*) f_data->Get(temp_tag);
	  std::sprintf(temp_tag, "Mll_%s", tag.data());
	  set.Mll[i]->SetName(temp_tag);
	  set.Mll[i]->SetDirectory(gROOT);
	  set.Mll[i]->SetLineColor(set.lineColor[i]);
	  set.Mll[i]->SetFillColor(set.color[i]);
	  set.Mll[i]->SetLineWidth(1);
	  // Mee
	  std::sprintf(temp_tag, "Mee_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // Mmumu
	  std::sprintf(temp_tag, "Mmumu_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // LEPPt
	  std::sprintf(temp_tag, "LEPPt_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // LEP1Pt
	  std::sprintf(temp_tag, "LEP1Pt_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // LEP2Pt
	  std::sprintf(temp_tag, "LEP2Pt_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // LEP3Pt
	  std::sprintf(temp_tag, "LEP3Pt_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  // LEPChargeSum
	  std::sprintf(temp_tag, "LEPChargeSum_%s_%s", processes[i].c_str(),tag.data());
	  ...
	  if(reduced){
		// lep_softest_pt
		std::sprintf(temp_tag, "lep_softest_pt_%s_%s", processes[i].c_str(),tag.data());
...
void getData_SigSignature(distrSigSignature& set, string tag, const char* signal_name, Int_t ind, const char* syst_tag) {
  ...
  getData_SigRegion(set.Incl, "PRE_" + tag, "", -1, syst_tag);
  if(!test_me){  getData_SigRegion(set.PRE, "PRE_" + tag, "", -1, syst_tag);
	getData_SigRegion(set.CR0b, "CR0b_" + tag, "", -1, syst_tag);
	getData_SigRegion(set.CR1b, "CR1b_" + tag, "", -1, syst_tag);
	getData_SigRegion(set.CR2b, "VRttW_" + tag, "", -1, syst_tag);
...
void getData_SigRegion(distrSigRegion& set, string tag, const char* signal_name, Int_t ind, const char* syst_tag) {
  ...
  if(!test_me){  getData_SigSet(set.fs_elel, "ee_" + tag, "", -1, syst_tag);
	getData_SigSet(set.fs_elmu, "em_" + tag, "", -1, syst_tag);
	getData_SigSet(set.fs_mumu, "mm_" + tag, "", -1, syst_tag);}
  getData_SigSet(set.fs_comb, "em_" + tag, "", -1, syst_tag);
...
void getData_SigSet(distrSigSet& set, string tag, const char* signal_name, Int_t ind, const char* syst_tag){
  ...
  const string files[N_SIG] = {"powhegpythia6TTBar"};

  for (int i = 0; i < N_SIG; i++){
	std::sprintf(temp_file, "%s/%sx0.root", path,files[i].c_str());
	set.file_name[i] = temp_file;
	set.color[i] = 0;
	set.lineColor[i] = 1;
  }
  
  set.lineColor[0] = kMagenta; // kRed
  ...
  for (int i = 0; i < N_SIG; i++) {
	...
	if(!test_me){
	  ...
	  // Mll
	  std::sprintf(temp_tag, "Mll_%s_mc_%s", files[i].c_str(), tag.data());
	  set.Mll[i] =(TH1D*) f_data->Get(temp_tag);
	  if(!draw_sig)
		set.Mll[i]->Reset();
	  set.Mll[i]->SetName(temp_tag);
...
void scaleLumi_Signature(distrSignature& set, Double_t sf) {
  ...
  if(!test_me){  scaleLumi_Region(set.PRE, sf);
...
void scaleLumi_Region(distrRegion& set, Double_t sf) {
  ...
  if(!test_me){  scaleLumi_Set(set.fs_elel, sf);
...
void scaleXSect_Signature(distrSignature& set, Int_t ind, Double_t sf) {
  ...
  if(!test_me){  scaleXSect_Region(set.PRE,  ind,  sf);
...
void scaleXSect_Region(distrRegion& set, Int_t ind, Double_t sf) {
  ...
  if(!test_me){  scaleXSect_Set(set.fs_elel, ind, sf);
...
void scaleXSect_SigSignature(distrSigSignature& set, Int_t ind, Double_t sf) {
  ...
  if(!test_me){  scaleXSect_SigRegion(set.PRE, ind, sf);
...
void scaleXSect_SigRegion(distrSigRegion& set, Int_t ind, Double_t sf) {
  ...
  if(!test_me){  scaleXSect_SigSet(set.fs_elel, ind, sf);
...
void weightData_Signature(distrSignature& set) {
  ...
  if(!test_me){  weightData_Region(set.PRE);
...
void weightData_Set(distrSet& set) {
  ...
  Double_t weight[N_DataMC];
  for (int i = 1; i < N_DataMC; i++) { // TO DO: start from 1 if 0 is data OR just create a new class for DATA or MC
...
void weightData_SigSignature(distrSigSignature& set) {
  ...
  if(!test_me){  weightData_SigRegion(set.PRE);
...
void weightData_SigRegion(distrSigRegion& set) {
  ...
  if(!test_me){  weightData_SigSet(set.fs_elel);
...
void buildStack(distrSet& set) {
  ...
  if(!test_me){
	...
	buildAStack(set.ST_Mll, set.Mll, &set.syst_tag);
...
void draw_Signature(distrSignature &set, distrSigSignature &sig, distrSignature* paired[N_unc_paired], distrSignature* unpaired[N_unc_unpaired], const char* fit){
  ...
  for (int j = 0; j < N_unc_paired; j++) {
	...
	if(!test_me){	paired_PRE[j] = (&(temp_set->PRE));
	  paired_CR0b[j] = (&(temp_set->CR0b));
	  paired_CR1b[j] = (&(temp_set->CR1b));
	  paired_CR2b[j] = (&(temp_set->CR0b));
	  paired_CR3b[j] = (&(temp_set->CR1b));}
  }

  for (int j = 0; j < N_unc_unpaired; j++) {
    distrSignature* temp_set = unpaired[j];
	unpaired_Incl[j] = (&(temp_set->Incl));
	if(!test_me){	unpaired_PRE[j] = (&(temp_set->PRE));
  ...
  if(!test_me){  draw_Region(set.PRE, sig.PRE, paired_PRE, unpaired_PRE, fit);
...
void draw_Region(distrRegion &set, distrSigRegion &sig,distrRegion* paired[N_unc_paired], distrRegion* unpaired[N_unc_unpaired], const char* fit){
  ...
  for (int j = 0; j < N_unc_paired; j++) {
	...
	if(!test_me){	paired_elel[j] = &(temp_set->fs_elel);
  ...
  for (int j = 0; j < N_unc_unpaired; j++) {
	...
	if(!test_me){	unpaired_elel[j] = &(temp_set->fs_elel);
	  unpaired_elmu[j] = &(temp_set->fs_elmu);
	  unpaired_mumu[j] = &(temp_set->fs_mumu);}
    unpaired_comb[j] = &(temp_set->fs_comb);
  }

  if(!test_me){  draw_Set(set.fs_elel, sig.fs_elel, paired_elel, unpaired_elel, fit);
...
void draw_Set(distrSet &set, distrSigSet &sig,  distrSet* paired[N_unc_paired], distrSet* unpaired[N_unc_unpaired], const char* fit){
  ...
  TObjArray *paired_Mll = new TObjArray(N_unc_paired);
  TObjArray *unpaired_Mll = new TObjArray(N_unc_unpaired);
  ...
  for (int j = 0; j < N_unc_paired; j++) {
	...
	if(!test_me){
	  ...
	  paired_Mll->Add(temp_set->ST_Mll);
  ...
  for (int j = 0; j < N_unc_unpaired; j++) {
	...
	if(!test_me){
	  ...
	  unpaired_Mll->Add(temp_set->ST_Mll);
  ...
  std::printf("draw merged\n");
  drawAVariable_merged(set.ST_NJETS, set.NJETS, sig.NJETS, paired_NJETS, unpaired_NJETS, fit);

  if(!test_me){
	//drawAVariable_normalized(set.ST_pt_jet1, set.pt_jet1, sig.pt_jet1, paired_pt_jet1, unpaired_pt_jet1, fit);
	drawAVariable_merged(set.ST_pt_jet1, set.pt_jet1, sig.pt_jet1, paired_pt_jet1, unpaired_pt_jet1, fit);
	//drawAVariable_normalized(set.ST_pt_jet2, set.pt_jet2, sig.pt_jet2, paired_pt_jet2, unpaired_pt_jet2, fit);
	drawAVariable_merged(set.ST_pt_jet2, set.pt_jet2, sig.pt_jet2, paired_pt_jet2, unpaired_pt_jet2, fit);
	//drawAVariable_normalized(set.ST_pt_jet3, set.pt_jet3, sig.pt_jet3, paired_pt_jet3, unpaired_pt_jet3, fit);
	drawAVariable_merged(set.ST_pt_jet3, set.pt_jet3, sig.pt_jet3, paired_pt_jet3, unpaired_pt_jet3, fit);
	if(reduced){
	  //drawAVariable_normalized(set.ST_pt_jet4, set.pt_jet4, sig.pt_jet4, paired_pt_jet4, unpaired_pt_jet4, fit);
	  drawAVariable_merged(set.ST_pt_jet4, set.pt_jet4, sig.pt_jet4, paired_pt_jet4, unpaired_pt_jet4, fit);
	  //drawAVariable_normalized(set.ST_pt_jet5, set.pt_jet5, sig.pt_jet5, paired_pt_jet5, unpaired_pt_jet5, fit);
	  drawAVariable_merged(set.ST_pt_jet5, set.pt_jet5, sig.pt_jet5, paired_pt_jet5, unpaired_pt_jet5, fit);
	}
	//drawAVariable_normalized(set.ST_NBJETS, set.NBJETS, sig.NBJETS, paired_NBJETS, unpaired_NBJETS, fit);
	drawAVariable_merged(set.ST_NBJETS, set.NBJETS, sig.NBJETS, paired_NBJETS, unpaired_NBJETS, fit);
	//drawAVariable_normalized(set.ST_pt_bjet1, set.pt_bjet1, sig.pt_bjet1, paired_pt_bjet1, unpaired_pt_bjet1, fit);
	drawAVariable_merged(set.ST_pt_bjet1, set.pt_bjet1, sig.pt_bjet1, paired_pt_bjet1, unpaired_pt_bjet1, fit);
	//drawAVariable_normalized(set.ST_pt_bjet2, set.pt_bjet2, sig.pt_bjet2, paired_pt_bjet2, unpaired_pt_bjet2, fit);
	drawAVariable_merged(set.ST_pt_bjet2, set.pt_bjet2, sig.pt_bjet2, paired_pt_bjet2, unpaired_pt_bjet2, fit);
	//drawAVariable_normalized(set.ST_pt_bjet3, set.pt_bjet3, sig.pt_bjet3, paired_pt_bjet3, unpaired_pt_bjet3, fit);
	drawAVariable_merged(set.ST_pt_bjet3, set.pt_bjet3, sig.pt_bjet3, paired_pt_bjet3, unpaired_pt_bjet3, fit);
	if(reduced){
	  //drawAVariable_normalized(set.ST_pt_bjet4, set.pt_bjet4, sig.pt_bjet4, paired_pt_bjet4, unpaired_pt_bjet4, fit);
	  drawAVariable_merged(set.ST_pt_bjet4, set.pt_bjet4, sig.pt_bjet4, paired_pt_bjet4, unpaired_pt_bjet4, fit);
	}
	//drawAVariable_normalized(set.ST_mu1_pt, set.mu1_pt, sig.mu1_pt, paired_mu1_pt, unpaired_mu1_pt, fit);
	drawAVariable_merged(set.ST_mu1_pt, set.mu1_pt, sig.mu1_pt, paired_mu1_pt, unpaired_mu1_pt, fit);
	//drawAVariable_normalized(set.ST_mu2_pt, set.mu2_pt, sig.mu2_pt, paired_mu2_pt, unpaired_mu2_pt, fit);
	drawAVariable_merged(set.ST_mu2_pt, set.mu2_pt, sig.mu2_pt, paired_mu2_pt, unpaired_mu2_pt, fit);
	//drawAVariable_normalized(set.ST_mu3_pt, set.mu3_pt, sig.mu3_pt, paired_mu3_pt, unpaired_mu3_pt, fit);
	drawAVariable_merged(set.ST_mu3_pt, set.mu3_pt, sig.mu3_pt, paired_mu3_pt, unpaired_mu3_pt, fit);
	//drawAVariable_normalized(set.ST_mu_eta, set.mu_eta, sig.mu_eta, paired_mu_eta, unpaired_mu_eta, fit);
	drawAVariable_merged(set.ST_mu_eta, set.mu_eta, sig.mu_eta, paired_mu_eta, unpaired_mu_eta, fit);
	//drawAVariable_normalized(set.ST_el1_pt, set.el1_pt, sig.el1_pt, paired_el1_pt, unpaired_el1_pt, fit);
	drawAVariable_merged(set.ST_el1_pt, set.el1_pt, sig.el1_pt, paired_el1_pt, unpaired_el1_pt, fit);
	//drawAVariable_normalized(set.ST_el2_pt, set.el2_pt, sig.el2_pt, paired_el2_pt, unpaired_el2_pt, fit);
	drawAVariable_merged(set.ST_el2_pt, set.el2_pt, sig.el2_pt, paired_el2_pt, unpaired_el2_pt, fit);
	//drawAVariable_normalized(set.ST_el3_pt, set.el3_pt, sig.el3_pt, paired_el3_pt, unpaired_el3_pt, fit);
	drawAVariable_merged(set.ST_el3_pt, set.el3_pt, sig.el3_pt, paired_el3_pt, unpaired_el3_pt, fit);
	//drawAVariable_normalized(set.ST_el_eta, set.el_eta, sig.el_eta, paired_el_eta, unpaired_el_eta, fit);
	drawAVariable_merged(set.ST_el_eta, set.el_eta, sig.el_eta, paired_el_eta, unpaired_el_eta, fit);
	//drawAVariable_normalized(set.ST_MET, set.MET, sig.MET, paired_MET, unpaired_MET, fit);
	drawAVariable_merged(set.ST_MET, set.MET, sig.MET, paired_MET, unpaired_MET, fit);
	//drawAVariable_normalized(set.ST_HT, set.HT, sig.HT, paired_HT, unpaired_HT, fit);
	drawAVariable_merged(set.ST_HT, set.HT, sig.HT, paired_HT, unpaired_HT, fit);
	//drawAVariable_normalized(set.ST_Meff, set.Meff, sig.Meff, paired_Meff, unpaired_Meff, fit);
	drawAVariable_merged(set.ST_Meff, set.Meff, sig.Meff, paired_Meff, unpaired_Meff, fit);
	//drawAVariable_normalized(set.ST_mT, set.mT, sig.mT, paired_mT, unpaired_mT, fit);
	drawAVariable_merged(set.ST_mT, set.mT, sig.mT, paired_mT, unpaired_mT, fit);
	//drawAVariable_normalized(set.ST_mT_min, set.mT_min, sig.mT_min, paired_mT_min, unpaired_mT_min, fit);
	drawAVariable_merged(set.ST_mT_min, set.mT_min, sig.mT_min, paired_mT_min, unpaired_mT_min, fit);
	if(reduced){
	  //drawAVariable_normalized(set.ST_MZ, set.MZ, sig.MZ, paired_MZ, unpaired_MZ, fit);
	  drawAVariable_merged(set.ST_MZ, set.MZ, sig.MZ, paired_MZ, unpaired_MZ, fit);
	  //drawAVariable_normalized(set.ST_Mlll, set.Mlll, sig.Mlll, paired_Mlll, unpaired_Mlll, fit);
	  drawAVariable_merged(set.ST_Mlll, set.Mlll, sig.Mlll, paired_Mlll, unpaired_Mlll, fit);
	}
	drawAVariable_merged(set.ST_Mll, set.Mll, sig.Mll, paired_Mll, unpaired_Mll, fit);
	//drawAVariable_normalized(set.ST_Mee, set.Mee, sig.Mee, paired_Mee, unpaired_Mee, fit);
	drawAVariable_merged(set.ST_Mee, set.Mee, sig.Mee, paired_Mee, unpaired_Mee, fit);
	//drawAVariable_normalized(set.ST_Mmumu, set.Mmumu, sig.Mmumu, paired_Mmumu, unpaired_Mmumu, fit);
	drawAVariable_merged(set.ST_Mmumu, set.Mmumu, sig.Mmumu, paired_Mmumu, unpaired_Mmumu, fit);
	//drawAVariable_normalized(set.ST_LEPPt, set.LEPPt, sig.LEPPt, paired_LEPPt, unpaired_LEPPt, fit);
	drawAVariable_merged(set.ST_LEPPt, set.LEPPt, sig.LEPPt, paired_LEPPt, unpaired_LEPPt, fit);
	//drawAVariable_normalized(set.ST_LEP1Pt, set.LEP1Pt, sig.LEP1Pt, paired_LEP1Pt, unpaired_LEP1Pt, fit);
	drawAVariable_merged(set.ST_LEP1Pt, set.LEP1Pt, sig.LEP1Pt, paired_LEP1Pt, unpaired_LEP1Pt, fit);
	//drawAVariable_normalized(set.ST_LEP2Pt, set.LEP2Pt, sig.LEP2Pt, paired_LEP2Pt, unpaired_LEP2Pt, fit);
	drawAVariable_merged(set.ST_LEP2Pt, set.LEP2Pt, sig.LEP2Pt, paired_LEP2Pt, unpaired_LEP2Pt, fit);
	//drawAVariable_normalized(set.ST_LEP3Pt, set.LEP3Pt, sig.LEP3Pt, paired_LEP3Pt, unpaired_LEP3Pt, fit);
	drawAVariable_merged(set.ST_LEP3Pt, set.LEP3Pt, sig.LEP3Pt, paired_LEP3Pt, unpaired_LEP3Pt, fit);
	//drawAVariable_normalized(set.ST_LEPChargeSum, set.LEPChargeSum, sig.LEPChargeSum, paired_LEPChargeSum, unpaired_LEPChargeSum, fit);
	drawAVariable_merged(set.ST_LEPChargeSum, set.LEPChargeSum, sig.LEPChargeSum, paired_LEPChargeSum, unpaired_LEPChargeSum, fit);
	if(reduced){
	  //drawAVariable_normalized(set.ST_lep_softest_pt, set.lep_softest_pt, sig.lep_softest_pt, paired_lep_softest_pt, unpaired_lep_softest_pt, fit);
...
void drawAVariable_normalized(THStack* &st, TH1D* hist[N_DataMC], TH1D* sig[N_SIG], TObjArray *paired, TObjArray *unpaired, const char* fit){
  ...
  for (int ilog=0; ilog<2; ilog++){
	...
	char fpath[512];
	char my_extension[256] = "";
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v50/Plots");
	int res = mkdir(fpath, 0700);
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v50/Plots/%s", folder);
	res = mkdir(fpath, 0700);
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v50/Plots/%s/DataMC", folder);
	res = mkdir(fpath, 0700);
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v50/Plots/%s/DataMC/normalized", folder);
	res = mkdir(fpath, 0700);
	if(ilog){
...
void drawAVariable_all(THStack* &st, TH1D* hist[N_DataMC], TH1D* sig[N_SIG], TObjArray *paired, TObjArray *unpaired, const char* fit){  
  ...
  for (int ilog=0; ilog<2; ilog++){
	...
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v50/Plots/%s/DataMC", folder);
	res = mkdir(fpath, 0700);
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v50/Plots/%s/DataMC/all", folder);
	res = mkdir(fpath, 0700);
	if(ilog){
	  sprintf(fpath, "%s/log", fpath);
	  res = mkdir(fpath, 0700);
...
void drawAVariable_merged(THStack* &st, TH1D* hist[N_DataMC], TH1D* sig[N_SIG], TObjArray *paired, TObjArray *unpaired, const char* fit){
  ...
  int ind=1;
  color[ind++] = kViolet-9; // ttW, ttZ -> note
  color[ind++] = 0;
  color[ind++] = kAzure+1;  // WW, WZ, ZZ -> note
  color[ind++] = 0;
  color[ind++] = 0;
  color[ind++] = kOrange-2; // tth, Vh
  color[ind++] = 0;
  color[ind++] = kSpring-6;// tt+WW/t/tt, tZ, VVV -> note
  color[ind++] = 0;
  color[ind++] = 0;
  color[ind++] = 0;
  color[ind++] = 0;
  color[ind++] = 0;
  color[ind++] = kGray+1; // WWos, Wt, single top
  color[ind++] = kOrange-9; // ttbar -> note
  color[ind++] = kOrange + 4; // Z+jets -> note
  color[ind++] = kRed-3; // W+jets

  for (int ilog=0; ilog<2; ilog++){
	
	// setup the merging machinery
	TH1D *my_hist[N_DataMC];
	char my_name[256];
	for (int i = 1; i < N_DataMC; i++)
	...
	std::cout << "Draw ratio = " << draw_ratio << std::endl;
	...
	if(draw_ratio){
	  ...
	  myPad2->SetPad(0, 0.0, 1, bt_fr2-0.01);
	...
	hist[0]->SetMarkerSize(1.2);
	hist[0]->GetXaxis()->SetLabelFont(42);
	hist[0]->GetXaxis()->SetLabelSize(0.05); //0.05
	hist[0]->GetXaxis()->SetTitleSize(0.05); //0.05
	hist[0]->GetXaxis()->SetTitleOffset(1.4);
	hist[0]->GetXaxis()->SetTitleFont(42);
	hist[0]->GetYaxis()->SetLabelFont(42);
	...
	err_brs->SetLineWidth(2);
	err_brs->SetMarkerSize(0);
    err_brs->SetLineColor(kRed);
	err_brs->SetFillColor(kBlack);
	err_brs->SetFillStyle(3005);

	for (Int_t ik = 1; ik <= err_brs->GetXaxis()->GetNbins()+1; ik++) {
      err_brs->SetBinError(ik,
						   sqrt(pow(err_brs->GetBinContent(ik)*.045,2)+ // Lumi!!
								pow(err_brs->GetBinError(ik),2)));
	  //	  std::cout << "bin=" << ik << " => " << err_brs->GetBinContent(ik) << " +- " << err_brs->GetBinError(ik) << std::endl;
	}

	hist[0]->Draw("axis 9");
	my_st->Draw("9 hist same");
	err_brs->Draw("9 same  E2");
	TH1F *hsave = (TH1F*)err_brs->Clone("hsave");
	hsave->SetFillColor(kWhite);
	...
	leg2 = new TLegend(0.65, 0.82, 0.87, 0.854782);
	if(draw_sig)
	...
	if (strstr(hist[0]->GetName(), "_SR3b1_") != NULL)
	  legendHeader += ", SR3b1, ";

	if (strstr(hist[0]->GetName(), "_SR3b2_") != NULL)
	  legendHeader += ", SR3b2, ";
	...
	if (strstr(hist[0]->GetName(), "_SR1b1_") != NULL)
	  legendHeader += ", SR1b1, ";

	if (strstr(hist[0]->GetName(), "_SR1b2_") != NULL)
	  legendHeader += ", SR1b2, ";
	...
	if (strstr(hist[0]->GetName(), "_SR3L_") != NULL)
	  legendHeader += ", SR3L, ";
	...
	if (strstr(hist[0]->GetName(), "_CR2b3L_") != NULL)
	  legendHeader += ", CR2b3L, ";

	if (strstr(hist[0]->GetName(), "_CR1bInclttZ_") != NULL)
	  legendHeader += ", CR1bInclttZ, ";

	if (strstr(hist[0]->GetName(), "_CR1bExclttZ_") != NULL)
	  legendHeader += ", CR1bExclttZ, ";

	if (strstr(hist[0]->GetName(), "_CR2bInclttZ_") != NULL)
	  legendHeader += ", CR2bInclttZ, ";

	if (strstr(hist[0]->GetName(), "_CR2bInclttV_") != NULL)
	  legendHeader += ", CR2bInclttV, ";

	if (strstr(hist[0]->GetName(), "_CR0bExclWWmjj_") != NULL)
	  legendHeader += ", CR0bExclWWmjj, ";

	if (strstr(hist[0]->GetName(), "_CR0bExclWZ_") != NULL)
	  legendHeader += ", CR0bExclWZ, ";
	...
	if(show_yield)
	  std::sprintf(temp, "SM Background (%.1e)", background->Integral());
	else
	  std::sprintf(temp, "SM Background");
	//	leg->AddEntry(background, temp , "flp");  
	leg->AddEntry(err_brs, temp , "flp");  
	...
	if(draw_ratio){
	  myPlots->cd(2);
	  myPad2->SetLogy(0);
	  //	  TPad *myPadRatio = new TPad("myPad1Ratio", "",0,0,1,0.33);
	  myPad2->Draw();
	  myPad2->cd();
	  ...
	  /*TH1D *TH1DaxisRatio = (TH1D*) hist[0]->Clone();
		...
		TH1DaxisRatio->GetXaxis()->SetLabelSize(0.12); //0.12
		TH1DaxisRatio->GetXaxis()->SetTitleSize(0.12); //0.12
		TH1DaxisRatio->GetXaxis()->SetTitleOffset(1.0);
		TH1DaxisRatio->GetXaxis()->SetTitleFont(42);
		TH1DaxisRatio->GetYaxis()->SetTitle("data/SM");
	  ...
	  error_band->SetFillColor(kYellow);
	  error_band->SetFillStyle(1001);
	  error_band->Draw("e2 9 same");

	  TH1D* stat_band = (TH1D*) background->Clone("Ratio_stat_band");
	  for (int k = 1; k <= stat_band->GetNbinsX(); k++)
		if (background->GetBinContent(k) != 0) {
		  stat_band->SetBinContent(k, stat_band->GetBinContent(k) / background->GetBinContent(k));
		  stat_band->SetBinError(k, stat_band->GetBinError(k) / background->GetBinContent(k));
		} else {
		  stat_band->SetBinContent(k, 1);
		  stat_band->SetBinError(k, 0);
		}
	  stat_band->SetMarkerSize(0);
	  stat_band->SetFillStyle(3005);
	  stat_band->SetFillColor(kBlack);
	  stat_band->Draw("e2 9 same");
	  ...
	  if (strstr(hist[0]->GetName(), "_OS") != NULL){
		line = new TLine(myPad1->GetUxmin(),1.1,myPad1->GetUxmax(),1.1);
		line->SetLineStyle(2);
		line->SetLineWidth(2);
		line->Draw();
		line = new TLine(myPad1->GetUxmin(),0.9,myPad1->GetUxmax(),0.9);
		line->SetLineStyle(2);
		line->SetLineWidth(2);
		line->Draw();
	  ...
	  Graph_g_hRatio3014->GetXaxis()->SetLabelSize(0.05); //0.05
	  Graph_g_hRatio3014->GetXaxis()->SetTitleSize(0.05); //0.05
	...
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v50/Plots/%s/DataMC", folder);
	res = mkdir(fpath, 0700);
	sprintf(fpath, "/afs/cern.ch/work/o/othrif/workarea/results/v50/Plots/%s/DataMC/merged", folder);
	res = mkdir(fpath, 0700);
	if(ilog){
	  sprintf(fpath, "%s/log", fpath);
	  res = mkdir(fpath, 0700);
	  sprintf(fpath, "%s/%s", fpath, signature.c_str());
	  printf("%s/%s", fpath, signature.c_str());
	  res = mkdir(fpath, 0700);
	  sprintf(my_extension, "%s/%s_log.pdf", fpath, hist[0]->GetName());
...
int main(int argc, char **argv ) {
  ...
  */
  ...
  //luminosity
  Float_t c_lumi_up = 1. + 0.045;//0.05;
  Float_t c_lumi_dn = 1. - 0.045;//0.05;
  ...
  //  scaleXSect_All(det_lvl, ttbar_ind, ( 696.11*1.1949*0.543 )/ 831.759766   );
  ...
  paired[ind_paired++] = &det_lvl;//pCF;
  paired[ind_paired++] = &det_lvl;//mCF;

  paired[ind_paired++] = &det_lvl;//pFrE;
  paired[ind_paired++] = &det_lvl;//mFrE;

  paired[ind_paired++] = &det_lvl;//pFrMu;
  paired[ind_paired++] = &det_lvl;//mFrMu;

  paired[ind_paired++] = &det_lvl;//pFlE;
  paired[ind_paired++] = &det_lvl;//mFlE;

  paired[ind_paired++] = &det_lvl;//pFlMu;
  paired[ind_paired++] = &det_lvl;//mFlMu;
