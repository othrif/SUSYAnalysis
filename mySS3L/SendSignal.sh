...
#out=`echo user.othrif.test.${dsID}.${dsName}.${tags}`
	out=`echo user.othrif.t0858_v55.${dsID}.${dsName}.${tags}`

	echo ${out}

	echo python SS3LAnalysis/scripts/Run.py --isData 0 --isAtlfast ${isAF} --ismc15c ${isMc15c} --driver grid --inputDS $i --outputDS ${out}; 
	python SS3LAnalysis/scripts/Run.py --isData 0 --isAtlfast ${isAF} --ismc15c ${isMc15c} --driver grid --inputDS $i --outputDS ${out}; 
