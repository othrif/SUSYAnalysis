...
#	isMc15c=`echo ${tags} | grep 'a818\|a821\|r7772\|r7725\|r8112\|r8129' | wc -l`
	...
	tags=`echo $i | cut -d"." -f7 | cut -d"_" -f1,3,4,5 `
	dsID=`echo $i | cut -d"." -f3`
    ...
    out=`echo user.othrif.x4.${dsID}.${dsName}.${tags}`

	echo ${out}

	echo python SS3LAnalysis/scripts/Run.py --isData 0 --isAtlfast ${isAF} --ismc15c 1 --driver grid --inputDS $i --outputDS ${out} --submitDir=$2; 
	python SS3LAnalysis/scripts/Run.py --isData 0 --isAtlfast ${isAF} --ismc15c 1 --driver grid --inputDS $i --outputDS ${out} --submitDir=$2; 
